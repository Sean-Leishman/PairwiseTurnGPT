...
from pairwisegpt.metrics import Metrics, MetricType
...
class Trainer:
    def __init__(
        ...
        **kwargs,
        ...
        self.sint_token_id = self.model.tokenizer.convert_tokens_to_ids("<sint>")
        self.eint_token_id = self.model.tokenizer.convert_tokens_to_ids("<eint>")
        ...
        self.eot_tokens = (
            torch.tensor(
                [
                    self.model.tokenizer.convert_tokens_to_ids("<ebc>"),
                    self.model.tokenizer.convert_tokens_to_ids("<eint>"),
        ...
        self.val_metrics = self.init_metrics(
            include_overlap_token,
            include_bc_token,
            include_end_bc_token,
            include_yield_token,
            "val",
        )
    ...
    def init_metrics(
        ...
        **kwargs,
        ...
        metrics = [
            ...
            (self.eot_token_id, "eot", [MetricType.BACC], TurnType.BACKCHANNEL),
            ...
            ("all", "int", [MetricType.BACC, MetricType.PR_AUC], TurnType.NONE),
        ...
        if (
            include_overlap_token
            and self.eint_token_id != self.model.tokenizer.pad_token_id
        ):
            metrics.append(
                (
                    self.eint_token_id,
        ...
        if (
            include_end_bc_token
            and self.ebc_token_id != self.model.tokenizer.pad_token_id
        ):
            metrics.append(
                (
                    self.ebc_token_id,
        ...
        if include_bc_token and self.bc_token_id != self.model.tokenizer.pad_token_id:
            metrics.append(
                (
                    self.bc_token_id,
                    "bc",
                    [MetricType.BACC, MetricType.F1_SCORE, MetricType.PR_AUC],
                    TurnType.NONE,
                )
            )
        if (
            include_yield_token
            and self.yield_token_id != self.model.tokenizer.pad_token_id
        ):
            metrics.extend(
                [
                    (
        ...
        tokens = {
            ...
            for k in self.model.tokenizer.special_tokens
        }
    ...
    def load_from_checkpoint(self):
        try:
            ...
            self.logger.info(f"model: loading parameters for model {checkpoint.keys()}")
    ...
    def train_epoch(self, train_dl):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            attention_maskA = batch["speakerA"]["attention_mask"].to(self.device)
            token_type_idsA = batch["speakerA"]["token_type_ids"].to(self.device)
            speaker_idsA = batch["speakerA"]["speaker_ids"].to(self.device)

            input_idsB = batch["speakerB"]["input_ids"].to(self.device)
            attention_maskB = batch["speakerB"]["attention_mask"].to(self.device)
            token_type_idsB = batch["speakerB"]["token_type_ids"].to(self.device)
            ...
            labelsA = self.generate_labels(input_idsA, mask=label_loss_maskA)
            projection_labelsA = self.generate_projection_labels(labelsA)

            labelsB = self.generate_labels(input_idsB, mask=label_loss_maskB)
        ...
        try:
            self.trp_example_plots(input_idsA, input_idsB, out, name="TRP/train")
    ...
    def validate(self, val_dl):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar):
                ...
                attention_maskA = batch["speakerA"]["attention_mask"].to(self.device)
                token_type_idsA = batch["speakerA"]["token_type_ids"].to(self.device)
                eot_typesA = batch["speakerA"]["other_token_type_ids"].to(self.device)
                turn_overlapA = batch["speakerA"]["turn_overlap"].to(self.device)
                speaker_idsA = batch["speakerA"]["speaker_ids"].to(self.device)

                input_idsB = batch["speakerB"]["input_ids"].to(self.device)
                attention_maskB = batch["speakerB"]["attention_mask"].to(self.device)
                token_type_idsB = batch["speakerB"]["token_type_ids"].to(self.device)
                eot_typesB = batch["speakerB"]["other_token_type_ids"].to(self.device)
                turn_overlapB = batch["speakerB"]["turn_overlap"].to(self.device)
                speaker_idsB = batch["speakerB"]["speaker_ids"].to(self.device)
                ...
                self.add_to_metrics(
                    out.logits[0].detach(), input_idsA.detach(), **metric_masksA
                )
                self.add_to_metrics(
                    out.logits[1].detach(), input_idsB.detach(), **metric_masksB
                )
    ...
    def test(self, test_dl, params):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar):
                ...
                emp_metric_masksA, emp_metric_masksB, emp_all_metric_masks = (
                    self.generate_masks(
                        input_idsA,
                        input_idsB,
                        token_type_idsA,
                ...
                self.add_to_metrics(
                    out.logits[0].detach(),
                    input_idsA.detach().clone(),
                    metric=self.emp_metrics,
                    **emp_metric_masksA,
                )
                self.add_to_metrics(
                    out.logits[1].detach(),
                    input_idsB.detach().clone(),
                    metric=self.emp_metrics,
                    **emp_metric_masksB,
                )
                ...
                self.add_to_metrics(
                    out.logits[0].detach(), input_idsA.detach().clone(), **metric_masksA
                )
                self.add_to_metrics(
                    out.logits[1].detach(), input_idsB.detach().clone(), **metric_masksB
                )
            ...
            emp_metrics, _, _ = self.emp_metrics.calculate(params, prefix="emp")
            ...
            if not self.dev_mode:
                ...
                wandb_out = {
                    "test_loss": round(total_loss / len(test_dl), 4),
                    "global_step": self.global_step,
                }
                ...
                self.trp_example_plots(input_idsA, input_idsB, out, name="TRP/test")
    ...
    def generate_projection_labels(self, labels):
        ...
        distances = torch.full(
            (batch_size, num_labels), num_labels, device=labels.device
        )
        distances[mask] = 0
    ...
    def save_history(self, path):
        ...
        for key in self.val_history:
            try:
                np.save(os.path.join(path, f"val_{key}"), self.val_history[key])
            except TypeError:
                print(f"Cannot save {key}")
    ...
    def add_to_bacc(self, logits, labels):
        ...
        for bacc in self.bacc_sot:
            ...
            bacc["bacc"] = (
                (bacc["tp"] / (bacc["tp"] + bacc["fn"]))
                + (bacc["tn"] / (bacc["fp"] + bacc["tn"]))
            ) / 2
    ...
    def generate_masks(
        ...
        if (
            not self.serialise_data and not self.no_emp_tokens
        ) or self.evaluate_on_full:
            # Overlapping where:
            ...
            non_yield_maskA = torch.logical_not(
                torch.logical_and(
                    ...
                    torch.eq(input_idsA, self.eot_token_id),
                )
            )
            non_yield_maskB = torch.logical_not(
                torch.logical_and(
                    torch.eq(eot_typesB, TurnType.YIELD),
                    torch.eq(input_idsB, self.eot_token_id),
                )
            )
            ...
            yield_maskA = torch.logical_not(
                torch.logical_and(
                    torch.ne(eot_typesA, TurnType.YIELD),
                    torch.eq(input_idsA, self.eot_token_id),
                )
            )
            yield_maskB = torch.logical_not(
                torch.logical_and(
                    torch.ne(eot_typesB, TurnType.YIELD),
                    torch.eq(input_idsB, self.eot_token_id),
                )
            )
            ...
            eot_items = torch.tensor([self.model.tokenizer.eos_token_id], device="cuda")
            if self.include_yield_token:
                eot_items = torch.tensor(
                    [self.yield_token_id, self.model.tokenizer.eos_token_id],
                    device="cuda",
                )
    ...
    def remove_special(self, input_ids, mask, token=None):
        ...
        for k, v in self.tokens_dict.items():
            mask = torch.logical_and(mask, torch.ne(input_ids, v))
        return mask

    def add_to_joint_metrics(
        self, logitsA, logitsB, input_idsA, input_idsB, metric=None, **kwargs
    ):
        if metric is None:
            self.metrics.add_joint(
                logitsA,
                logitsB,
                input_idsA=input_idsA,
                input_idsB=input_idsB,
